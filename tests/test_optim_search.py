...
""",
    ...
    p = Project(name="Opt", root_dir=str(tmp_path))
...
def test_deterministic_search_outputs_ranked_artifact(tmp_path: Path):
    path = _seed(tmp_path)
    res = run_deterministic_search(path, "j1", max_rows=2, max_cols=2, dimming_levels=[0.8, 1.0], top_n=3)
    assert res.best.rank == 1
    assert len(res.top) == 3
    assert Path(res.artifact_json).exists()
    assert len(res.best_layout) >= 1
