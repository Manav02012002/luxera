...
def _scene_dir() -> Path:
    return Path(__file__).resolve().parent / "scenes" / "corridor"
...
def _mock_radiance(monkeypatch: pytest.MonkeyPatch, cpu_vals: np.ndarray, scale: float = 0.99) -> None:
...
def _assert_translation_invariance(scene_project_path: Path) -> None:
    ...
    r0 = run_job_in_memory(p0, "j_direct")
    v0 = _grid_values(r0.result_dir)

    p1 = load_project_schema(scene_project_path)
    p1 = copy.deepcopy(p1)
    _absolutize_asset_paths(p1, scene_dir)
    dx, dy = 8.0, 5.0
    ...
    r1 = run_job_in_memory(p1, "j_direct")
    v1 = _grid_values(r1.result_dir)
    assert np.allclose(v0, v1, rtol=1e-5, atol=1e-5)


def test_direct_vs_radiance_corridor(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    scene = _scene_dir()
    ...
    cpu_ref = run_job_in_memory(project, "j_direct")
    cpu_vals = _grid_values(cpu_ref.result_dir)

    _mock_radiance(monkeypatch, cpu_vals, scale=0.99)
    out_dir = tmp_path / "corridor_rad"
    out_dir.mkdir()
