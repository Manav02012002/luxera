...
def _scene_dir() -> Path:
    return Path(__file__).resolve().parent / "scenes" / "l_shape"
...
def test_direct_vs_radiance_l_shape(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    scene = _scene_dir()
