...
def _scene_dir() -> Path:
    return Path(__file__).resolve().parent / "scenes" / "box_room"
...
def _mock_radiance(monkeypatch: pytest.MonkeyPatch, cpu_vals: np.ndarray, scale: float = 1.01) -> None:
...
def test_direct_vs_radiance_box(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    scene = _scene_dir()
    ...
    _mock_radiance(monkeypatch, cpu_vals, scale=1.01)
    out_dir = tmp_path / "box_rad"
    out_dir.mkdir()
