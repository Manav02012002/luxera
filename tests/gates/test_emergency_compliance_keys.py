...
def test_emergency_compliance_artifact_keys(tmp_path: Path) -> None:
    ies = _ies_fixture(tmp_path / "em.ies")
    p = Project(name="EmergencyKeys", root_dir=str(tmp_path))
    ...
    p.jobs.append(
        JobSpec(
            ...
            mode=EmergencyModeSpec(emergency_factor=0.4),
            routes=["r1"],
    ...
    comp = json.loads((out_dir / "emergency_compliance.json").read_text(encoding="utf-8"))
    assert "standard" in comp
    assert "thresholds" in comp
    assert "pass_fail" in comp
